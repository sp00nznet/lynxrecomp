...
void (*lynx_dispatch_fallback)(uint16_t) = 0;
...
void lynx_call_addr(uint16_t addr) {
    ...
    else if (lynx_dispatch_fallback) lynx_dispatch_fallback(addr);  /* interpret gap */
    else lynx_last_ext_addr = addr;                  /* record + no-op              */
}
